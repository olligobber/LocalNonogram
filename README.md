...
* `src/solveHints.sh`: This is the source for `exe/solveHints`. Similar to `src/makeHints.sh`, a function is used to solve each hint, which is then efficiently applied to each line. However, to help handle the complexity of the task, a lot of extra code was used. This includes a class for deductions featuring methods for "either" and "both", classes for monads that support reading and updating the knowledge about the grid, and a basic implementation of those classes.